...
func (le *linkExtractor) Process(ctx context.Context, p pipeline.Payload) (pipeline.Payload, error) {
	...
	if len(baseMatch) == 2 {
		base := resolveURL(relTo, ensureHasTrailingSlash(baseMatch[1]))
		if base != nil {
			relTo = base
		}
	}

	// Find the unique set of links from the document, resolve them, and add them to the payload
...
func (le *linkExtractor) retainLink(srcHost string, link *url.URL) bool {
	...
	isPrivate, err := le.netDetector.IsPrivate(link.Host)
...
// - targets starting with '//' are treated as absolute URLs that inherit the protocol relTo
// - targets starting with '/' are absolute URLs that are appended to the host from relTo
