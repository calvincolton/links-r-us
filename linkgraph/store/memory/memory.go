...
// InMemoryGraph implements an in-memory link graph that can be
...
// Links returns an iterator for the set of links whose IDs belong to the same
...
func (s *InMemoryGraph) UpsertEdge(edge *graph.Edge) error {
	...
	for _, edgeID := range s.linkEdgeMap[edge.Src] {
		exisatingEdge := s.edges[edgeID]
		if exisatingEdge.Src == edge.Src && exisatingEdge.Dst == edge.Dst {
			exisatingEdge.UpdatedAt = time.Now()
			*edge = *exisatingEdge
			return nil
		}
	}
...
// belong to the [fromID, toID] range and were updated before the provided timestamp
