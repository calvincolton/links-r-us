...
// Each input is passed to the specified processor and its output is emitted to the next stage
...
// FixedWorkerPool returns a StageRunner that spins up a pool containing numWorkers
// to process incoming payloads in parallel and emit their outputs to th enext stage
...
// Run implements fixedWorkerPool StageRunner
...
// that can scale up to maxWorkers for processing incoming inputs in parallel and emitting their outputs to the next stage
...
// Run implements dynamicWorkerPool StageRunner
	...
	for {
		...
		case payloadIn, ok := <-params.Input():
			...
			go func(payloadIn Payload, token struct{}) {
				...
				// If the processor did not ouput a payload for the next stage, there is nothing we need to do
				if payloadOut == nil {
					payloadIn.MarkAsProcessed()
					return
				}
...
func (b *broadcast) Run(ctx context.Context, params StageParams) {
	...
	// Start each FIFO in a go-routine. Each FIFO gets its own dedicated input channel and shared output channel passed to Run
	...
	for {
		...
		case payload, ok := <-params.Input():
			...
			for i := len(b.fifos) - 1; i >= 0; i-- {
				// As each FIFO might modify the payload, to avoid data races
				// we need to make a copy of the payload for all FIFOs except the first
