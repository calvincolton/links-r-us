...
// Pipeline implements a moduler, multi-stage pipeline.
// Each pipeline is constructed out of an input source, an output sink, and zero or more processing stages
type Pipeline struct {
	stages []StageRunner
}

// New returns a new pipeline instance where input payloads will traverse one of the specified stages
...
// Process reads the contents of a specified source, sends them through various stages of the pipeline
// and directs the results to the specified sink, and returns back any errors that have occurred
//
...
func (p *Pipeline) Process(ctx context.Context, source Source, sink Sink) error {
	...
	// Allocate channels for wiring together the source
...
// and pushes them to an output channel that is used as input for the first stage of the pipeline
...
// sinkWorker implements a worker that reads Payload instances from an input channel
// (the output of the last pipeline stage) and passes them to the provided sink
