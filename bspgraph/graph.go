...
import (
	...
	"github.com/calvincolton/links-r-us/bspgraph/message"
...
// Freeze marks the vertex as inactive. Inactive vertices will not be processed in the following supersteps
// unless they receive a message in which case they will be re-activated
...
// It is important for callers to invoke Close() on the returned graph instnace when they are done using it
...
// Reset the state of the graph by removing any existing vertices or aggregators and reseeting the superstep counter
...
// AddVertex inserts a new vertex witht he specified id and initial value into the graph
// If the vertex already exists, AddVertex will just overwrite its value with the provided initValue
...
// Aggregator returns the aggregator with the specified name or nil if the aggregator does not exists
...
// RegisterRelayer configures a Relayer that tbe graph will invoke when attempting to deliver a message to a vertex that is not known locally but could potentially be owned by a remote graph instance
func (g *Graph) RegisterRelayer(relayer Relayer) { g.relayer = relayer }

// BroadcastToNeighbors is a helper function that broadcasts a single message to each neighbor of a particular vertex
// Messages are queued for delivery and will be processed by recipients in the next superstep
...
// SendMessage attempts to deliver a message to the vertex with the specified destination ID.
// Messages are queued for delivery and will be processed by recipients in the next superstep
func (g *Graph) SendMessage(dstID string, msg message.Message) error {
	// If the vertex is known to the local graph instance, queue the message directly so it can be delivered at the next superstep
	...
	// The vertex is not known locally but might be known to a partition that is processed at another node.
	// If a remote relayer has been configured, delegate the message sned operation to it
	if g.relayer != nil {
...
// Step executes the next superstep and returns back the number of vertices that were processed either because they were still active or they received a message
...
// startWorkers allocates the required channels and spins up numWorkers to execute each superstep
...
// stepWorker polls vertexCh for incoming vertices and executes the configured ComputeFunc for each one
// The worker automatically exits when vertexCh gets closed
func (g *Graph) stepWokrer() {
